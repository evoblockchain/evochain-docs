...
The main command used to set up `evochaincli` is the following:
...
# example: evochaincli config node https://evochaintesttmrpc.evoblock.com (testnet)
# example: evochaincli config node https://evochaintmrpc.evoblock.com (mainnet)
```
...
# mainnet
evochaincli config chain-id evochain-66
```

Set the output format of evochaincli request response
```bash
evochaincli config output json    
evochaincli config indent true
```

## evochaincli user manual
...
```bash
evochaincli keys add <name> [flags]
```
#### Successful response:
```
evochaincli keys add wind
  
# Example output
{
  "name": "wind",
  "type": "local",
  "address": "evo14tytg8a9fascnr4ruhszlflunyew5rw9qclx4n",
...
evochaincli keys add --recover admin   -y -m "right anxiety future wish fall carpet script old alert entire remain purse"
```
...
```
  ...
  [
  {
    "name": "alice",
    "type": "local",
    "address": "evo19n6w5l0htdgn2zwet9rtgvrzuf4a3qp4znwfcn",
  ...
  },
  {
    "name": "bob",
    "type": "local",
    "address": "evo1qpel9c5wlrc30efaskqfgzrda7h3sd74ngnx6m",
...
```bash
evochaincli keys show [name [name...]] [flags]
```
#### Successful response:
```
  evochaincli keys show bob
...
```bash
evochaincli keys delete <name> [flags]
```
#### Successful response:
```
  evochaincli keys delete bob
...
```bash
evochaincli keys update <name> [flags]
```
#### Successful response:
```
  evochaincli keys update bob
...
```bash
evochaincli keys mnemonic [flags]
```
#### Successful response:
```
evochaincli keys mnemonic
...
#### Parameter description:
  ...
  | Parameter |                         Mark                          |
  | :-------: | :----------------------------------------------------------: |
  |  Address  | User address (eg. evo19n6w5l0htdgn2zwet9rtgvrzuf4a3qp4znwfcn) |

#### Example:
Query user information, including serial numbers, public keys and token balances
```bash
evochaincli query account <address>
```
#### Successful response:
```
  evochaincli query account evo19n6w5l0htdgn2zwet9rtgvrzuf4a3qp4znwfcn
...
```bash
evochaincli tx send [from_key_or_address] [to_address] [amount] [flags]
```
...
| amount |     string      | transfer amount, covering various cryptocurrencies separated by commas, eg. 1evo, 2bcoin |
...
```bash
evochaincli tx send alice evo1fh9tpkqka29n0mj307cu5cvp5ts0p4dl3mkv7r 2evo --from alice --gas-prices 0.00000001evo --gas auto --gas-adjustment 1.5 -b block
```
#### Successful response:
```
{
...
Issue a new token
```bash
evochaincli tx token issue [flags]
```
#### Successful response:
```
evochaincli tx token issue --from alice --symbol bcoin -n 200000 -w 'bcoin' --desc 'blockchain coin' --gas-prices 0.00000001evo --gas auto --gas-adjustment 1.5 -b block --mintable  
# example  
{
  "height": "340949",
...
```bash
evochaincli tx token mint [flags]
```
#### Successful response:
```
evochaincli tx token mint --amount 10000000 --symbol evo --from alice --gas-prices 0.00000001evo --gas auto --gas-adjustment 1.5 -b block
# Example output
{
  "height": "341001",
...
```bash
evochaincli tx token burn [flags]
```
#### Successful response:
```
evochaincli tx token burn --from alice --symbol evo --amount 100.0 --gas-prices 0.00000001evo --gas auto --gas-adjustment 1.5 -b block
  # Example output
{
  "height": "341036",
...
```bash
  evochaincli query token info <symbol>
```
#### Successful response:
```
  # Example output
  {
    ...
    "wholeName": "bcoin",
    "totalSupply": "200000",
    "owner": "evo19n6w5l0htdgn2zwet9rtgvrzuf4a3qp4znwfcn",
    "mintable": true
  }
```
...
```bash
evochaincli tx token multi-send [flags]
```
#### Successful response:

```
  evochaincli tx token multi-send --from alice --transfers '[{"to":"evo1fh9tpkqka29n0mj307cu5cvp5ts0p4dl3mkv7r","amount":"1evo,2btc-254"}]' --gas-prices 0.00000001evo --gas auto --gas-adjustment 1.5 -b block
...
Example:
```bash
evochaincli keys add --pubkey=expub1addwnpepqtg367t3j6myh4ces0luq3f6g87ptzwszpl9g5r28tgavypkdmm2w5l4zuq p1

evochaincli keys add --pubkey=expub1addwnpepqg334a4my6ufrs7r0ajsd6lxac9arsvtqljf0fzrgr27xvf3n5uugpsxna8 p2

evochaincli keys add --pubkey=expub1addwnpepqd7jd60n88tk98hyh72xsw48pjpfhdw0cd77ju59eqc88sxscfjkgx7tyfc p3
```
...
```bash  
evochaincli keys add --multisig-threshold=2 --multisig=p1,p2,p3 p1p2p3
```
...
Display user addresses p1, p2, p3 and deposit 100.1 evo into them
* Example:
```bash
evochaincli keys show -a p1p2p3
evochaincli tx send evo14tytg8a9fascnr4ruhszlflunyew5rw9qclx4n 100.1evo --from=alice
evochaincli query account evo14tytg8a9fascnr4ruhszlflunyew5rw9qclx4n
...
###### Example:
```bash
evochaincli tx send evo1fh9tpkqka29n0mj307cu5cvp5ts0p4dl3mkv7r 10evo \
  --chain-id=evochain-65 \
  --from=evo14tytg8a9fascnr4ruhszlflunyew5rw9qclx4n \
...
```bash
evochaincli tx sign \
  --multisig=evo14tytg8a9fascnr4ruhszlflunyew5rw9qclx4n \
  --from=alice \
...
##### Example:
```bash
evochaincli tx token transfer-ownership [flags]
```
##### Successful response:
```
# from alice to jack
evochaincli tx token transfer-ownership --from evo19n6w5l0htdgn2zwet9rtgvrzuf4a3qp4znwfcn --to evo1hcngft7gfkhn8z8fnlajzh7agyt0az0v6ztmme --symbol evo
  ...
  "events": [
    {
      ...
      "attributes": [
        {
          "key": "sender",
          "value": "evo19n6w5l0htdgn2zwet9rtgvrzuf4a3qp4znwfcn"
        },
        {
          "key": "module",
          "value": "token"
    ...
    },
    {
      ...
      "attributes": [
        {
          "key": "recipient",
          "value": "evo1qj5c07sm6jetjz8f509qtrxgh4psxkv3ddyq7u"
        },
        {
          "key": "amount",
          "value": "10.00000000evo"
        },
        {
          "key": "recipient",
          "value": "evo1qj5c07sm6jetjz8f509qtrxgh4psxkv3ddyq7u"
...
##### Example:
```bash
evochaincli tx token confirm-ownership [flags]
```
##### Successful response:
```
evochaincli tx token confirm-ownership --from evo1hcngft7gfkhn8z8fnlajzh7agyt0az0v6ztmme --symbol evo
  ...
  "events": [
    {
      ...
      "attributes": [
        ...
        {
          "key": "sender",
          "value": "evo1hcngft7gfkhn8z8fnlajzh7agyt0az0v6ztmme"
        }
      ]
    },
    {
...
```bash
$ evochaincli tx dex  -h

Usage:
  evochaincli tx dex [command]
...
```shell
$ evochaincli tx dex list -h

Example:
    evochaincli tx dex list --base-asset tusdk-9a2 --quote-asset tbtc-965 --from mykey --gas-prices 0.00000001evo --gas auto --gas-adjustment 1.5 -b block -y

Usage:
    evochaincli tx dex list [flags]
...
Flags:
  ...
  -h, --help                    help for list
      ...
      --quote-asset string      quote-asset should be issued before listed to opendex (default "evo")
  -y, --yes                     Skip tx broadcasting prompt confirmation

```

### Deposit an amount of evo on a token pair

```shell
$ evochaincli tx dex deposit -h

Example:
    evochaincli tx dex deposit tusdk-9a2_tbtc-965 100evo --from mykey --gas-prices 0.00000001evo --gas auto --gas-adjustment 1.5 -b block -y

Usage:
    evochaincli tx dex deposit [product] [amount] [flags]
...
### Withdraw an amount of evo from a token pairs

```shell
$ evochaincli tx dex  withdraw -h

Example:
    evochaincli tx dex withdraw tusdk-9a2_tbtc-965 50evo --from mykey --from mykey --gas-prices 0.00000001evo --gas auto --gas-adjustment 1.5 -b block -y

Usage:
    evochaincli tx dex withdraw [product] [amount] [flags]
...
##### Example:
```bash
evochaincli tx dex transfer-ownership [flags]
```
##### Successful response:
```
# from alice to jack
evochaincli tx dex transfer-ownership --from evo19n6w5l0htdgn2zwet9rtgvrzuf4a3qp4znwfcn --to evo1qpel9c5wlrc30efaskqfgzrda7h3sd74ngnx6m --product btc_evo
...
##### Example:
```bash
evochaincli tx dex confirm-ownership [flags]
```
##### Successful response:
```
evochaincli tx dex confirm-ownership --from evo1hcngft7gfkhn8z8fnlajzh7agyt0az0v6ztmme --product btc_evo
...
```shell
$ evochaincli query dex products -h

Example:
    evochaincli query dex products -p 1 -i 1

Usage:
    evochaincli query dex products [flags]
...
```shell
$ evochaincli query dex deposits -h

Example:
    evochaincli query dex deposits evo14tytg8a9fascnr4ruhszlflunyew5rw9qclx4n -i 1

Usage:
    evochaincli query dex deposits [account-addr] [flags]
...
```shell
$ evochaincli query dex match-order -h

Example:
    evochaincli query dex match-order -i 1 --node ${url}

Usage:
    evochaincli query dex match-order [flags]
...
```shell
$ evochaincli query dex  params -h

Example:
    evochaincli query dex  params --node ${url}

Usage:
    evochaincli query dex params [flags]
...
```shell
$ evochaincli query dex  products-delisting -h

Example:
    evochaincli query dex  products-delisting --node ${url}

Usage:
    evochaincli query dex products-delisting [flags]
```
...
```bash
# place orders
evochaincli tx order new [flags]
```
```
Flags:
  ...
  -p, --price string            The price of the order
      --product string          Trading pair in full name of the tokens: ${baseAssetSymbol}_${quoteAssetSymbol}, for example "mycoin_evo".
...
Global Flags:
  ...
  -e, --encoding string   Binary encoding (hex|b64|btc) (default "hex")
      --home string       directory for config and data (default "/Users/dwil/.evochaincli")
...
```
# example
evochaincli tx order new --product xxb-08a_evo,xxb-08a_evo -s BUY,BUY -p 1,1 -q 1,1 --from captain --gas-prices="0.00000001evo" --gas "200000" -y -b block
# example return
{
  "height": "373",
  ...
  "events": [
    ...
    },
    {
      ...
      "attributes": [
        ...
        {
          "key": "recipient",
          "value": "evo14tytg8a9fascnr4ruhszlflunyew5rw9qclx4n"
        },
        {
          "key": "amount",
          "value": "0.00200000evo"
...
```bash
evochaincli tx order cancel [order-id] [flags]
```
```
Flags:
  -a, --account-number uint     The account number of the signing account (offline mode only)
...
```
# example
evochaincli tx order cancel ID0000000373-1,ID0000000373-2 --from captain --gas-prices="0.00000001evo" --gas "200000" -y -b block
  ...
  "events": [
    {
      ...
      "attributes": [
        ...
        {
          "key": "orders",
          "value": "[{\"code\":0,\"msg\":\"0.00000000evo\",\"orderid\":\"ID0000000373-1\"},{\"code\":0,\"msg\":\"0.00000000evo\",\"orderid\":\"ID0000000373-2\"}]"
        },
        {
          "key": "action",
          "value": "cancel"
...
```bash
evochaincli query order detail [orderId]
```
```
Flags:
...
```
# example
evochaincli query order detail ID0000000525-1
# example return
{
	"txhash": "B32EC706A2B5ACCF4FC43D6530588C0C9E36E79AC572D3443653281500D46FDA",
	"order_id": "ID0000000525-1",
	"sender": "evo19n6w5l0htdgn2zwet9rtgvrzuf4a3qp4znwfcn",
	"product": "xxb-08a_evo",
	"side": "BUY",
	"price": "1.00000000",
	"quantity": "1.00000000",
	...
	"extra_info": "{\"newFee\":\"0.00000000evo\"}"
}
```
#### 2.2 Order depthbook:
...
##### Example:
```bash
evochaincli query order depthbook [product]
```
```
Flags:
...
```
# example
evochaincli query order depthbook mycoin_evo
# example return
{
	"asks": null,
	"bids": [{
...
#### Example:
```bash
evochaincli backend orders [open/closed] [addr] [flags]
...
```
# example
evochaincli backend orders open $(evochaincli keys show bob -a)
# example return
{
	"code": 0,
	"msg": "",
	...
	"data": {
		"data": [{
			...
			"sender": "evo1qpel9c5wlrc30efaskqfgzrda7h3sd74ngnx6m",
			"product": "mycoin_evo",
			"side": "SELL",
			"price": "11.10000000",
			"quantity": "1.00000000",
...
#### Example:
```bash
evochaincli backend deals [flags]
...
```
# example
evochaincli backend deals --address=$(evochaincli keys show bob -a)
# example return
{
	"code": 0,
	"msg": "",
...
#### Example:
```bash
evochaincli backend fees [addr] [flags]
...
```
# example
evochaincli backend fees $(evochaincli keys show bob -a)
# example return
{
	"code": 0,
	"msg": "",
...
#### Example:
```bash
evochaincli backend matches [flags]
...
#### Example:
```bash
evochaincli backend txs [addr] [flags]
...
```
# example
evochaincli backend txs $(evochaincli keys show bob -a)
# example return
{
	"code": 0,
	"msg": "",
...
#### Example:
```bash
evochaincli backend block_tx_hashes [height] [flags]
...
```
# example
evochaincli backend block-tx-hashes 105
# example return
...
#### Example:
```bash
evochaincli backend klines [flags]
...
Flags:
  ...
  -h, --help              help for klines
      --node string       <host>:<port> to tendermint rpc interface for this chain (default "tcp://localhost:26657")
  -p, --product string    product of coin pairs (default "evo_xxx")
...
```
# example
evochaincli backend klines -p mycoin_evo 
...
#### Example:
```bash
evochaincli backend tickers [flags]

Flags:
  -c, --count int        ticker count (default 10)
  -h, --help             help for tickers
      --node string      <host>:<port> to tendermint rpc interface for this chain (default "tcp://localhost:26657")
  -p, --product string   product of coin pairs (default "mycoin_evo")
...
```
# example
evochaincli backend tickers -p mycoin_evo
...
```shell
$ evochaincli tx staking create-validator -h

Example:
    evochaincli tx staking create-validator --moniker $MONIKER --pubkey $PUBKEY --from mykey
    
Usage:
    evochaincli tx staking create-validator [flags]
...
```shell
$ evochaincli tx staking destroy-validator -h
  
Example:
    evochaincli tx staking destroy-validator --from mykey
    
Usage:
    evochaincli tx staking destroy-validator [flags]
...
```shell
$ evochaincli tx staking edit-validator -h

Example:
    evochaincli tx staking edit-validator --moniker "my new name" --identity "http://mynewwebsite/pic/newlogo.jpg" --website "http://mynewwebsites" --details "my new slogans" --from mykey
	
Usage:
    evochaincli tx staking edit-validator [flags]
...
Delegate an amount of evo

```shell
$ evochaincli tx staking delegate -h

Example:
	  evochaincli tx staking delegate 1024.1024evo --from mykey
	  
Usage:
    evochaincli tx staking delegate [amount] [flags]
...
Vote on one or more validator(s) by delegate evo

Vote on one or more validator(s)

```shell
$ evochaincli tx staking vote -h

 
Example:
    evochaincli tx staking vote evovaloper1alq9na49n9yycysh889rl90g9nhe58lcs50wu5,evovaloper1svzxp4ts5le2s4zugx34ajt6shz2hg42a3gl7g,evovaloper10q0rk5qnyag7wfvvt7rtphlw589m7frs863s3m,evovaloper1g7znsf24w4jc3xfca88pq9kmlyjdare6mph5rx --from mykey

Usage:
    evochaincli tx staking vote [validator-addr1, validator-addr2, validator-addr3, ... validator-addrN] [flags]
```
...
```shell
$ evochaincli tx staking unbond -h

Example:
    evochaincli tx staking unbond 1024.1024evo --from mykey

Usage:
    evochaincli tx staking unbond [amount] [flags]
...
```shell
$ evochaincli query staking delegator -h

Example:
    evochaincli query staking delegator evo19n6w5l0htdgn2zwet9rtgvrzuf4a3qp4znwfcn
    
Usage:
    evochaincli query staking delegator [address] [flags]
...
```shell
$ evochaincli query staking votes-to -h

Example:
	  evochaincli query staking votes-to evovaloper1alq9na49n9yycysh889rl90g9nhe58lcs50wu5
	  
Usage:
    evochaincli query staking votes-to [validator-addr] [flags]
...
```
Usage:
    evochaincli query staking [command] [flags]
...
```shell
$ evochaincli tx staking proxy reg -h

Example:
	  evochaincli tx staking proxy reg --from mykey
	  
Usage:
	  evochaincli tx staking proxy reg [flags]
...
```shell
$ evochaincli tx staking proxy unreg -h

Example:
	  evochaincli tx staking proxy unreg --from mykey
	  
Usage:
    evochaincli tx staking proxy unreg [flags]
...
```shell
$ evochaincli tx staking proxy bind -h

Example:
	  evochaincli tx staking proxy bind evo19n6w5l0htdgn2zwet9rtgvrzuf4a3qp4znwfcn --from mykey
	  
Usage:
    evochaincli tx staking proxy bind [flags]
...
```shell
$ evochaincli tx staking proxy unbind -h

Example:
	  evochaincli tx staking proxy unbind evo19n6w5l0htdgn2zwet9rtgvrzuf4a3qp4znwfcn --from mykey
	  
Usage:
    evochaincli tx staking proxy unbind [flags]
...
```shell
$ evochaincli query staking proxy -h

Example:
	  evochaincli query staking proxy evo19n6w5l0htdgn2zwet9rtgvrzuf4a3qp4znwfcn
	  
Usage:
    evochaincli query staking proxy [address] [flags]
...
```shell
$ evochaincli tx distr withdraw-rewards -h

Example：
    evochaincli tx distr withdraw-rewards evovaloper1g7znsf24w4jc3xfca88pq9kmlyjdare6mph5rx --from ${name} --node ${url}

Usage: 
    evochaincli tx distr withdraw-rewards [validator-address] [flags]
...
```shell
$ evochaincli query distr params -h

Example：
    evochaincli query distr params --node ${url}
    
Usage: 
    evochaincli query distr params [flags]
...
```shell
$ evochaincli query distr community-pool -h

Example：
    evochaincli query distr community-pool --node ${url}
    
Usage: 
    evochaincli query distr community-pool [flags]
...
```shell
$ evochaincli query distr commission -h

Example：
    evochaincli query distr commission evovaloper1g7znsf24w4jc3xfca88pq9kmlyjdare6mph5rx --node ${url}
    
Usage: 
    evochaincli query distr commission [validator] [flags]
...
```shell
$ evochaincli tx gov submit-proposal delist-proposal -h

Submit a dex delist proposal along with an initial deposit.
The proposal details must be supplied via a JSON file.

Example:
  evochaincli tx gov submit-proposal delist-proposal <path/to/proposal.json> --from=<key_or_address>

Where proposal.json contains:

{
 "title": "delist xxx/evo",
...
Usage:
  evochaincli tx gov submit-proposal delist-proposal [proposal-file] [flags]

Flags:
  -a, --account-number uint     The account number of the signing account (offline mode only)
  -b, --broadcast-mode string   Transaction broadcasting mode (sync|async|block) (default "sync")
      --dry-run                 ignore the --gas flag and perform a simulation of a transaction, but don't broadcast it
      --fees string             Fees to pay along with transaction; eg: 1evo
      --from string             Name or address of private key with which to sign
      --gas string              gas limit to set per-transaction; set to "auto" to calculate required gas automatically (default 200000) (default "200000")
      --gas-adjustment float    adjustment factor to be multiplied against the estimate returned by the tx simulation; if the gas limit is set manually this flag is ignored  (default 1)
      --gas-prices string       Gas prices to determine the transaction fee (e.g. 1evo)
      --generate-only           Build an unsigned transaction and write it to STDOUT (when enabled, the local Keybase is not accessible and the node operates offline)
...
Global Flags:
      --chain-id string   Chain ID of tendermint node
  -e, --encoding string   Binary encoding (hex|b64|btc) (default "hex")
      --home string       directory for config and data (default "/Users/admin/.evochaincli")
  -o, --output string     Output format (text|json) (default "text")
      --passwd string     Pass word of sender (default "12345678")
      --trace             print out full stack trace on errors
```
...
```shell
$ evochaincli tx gov  submit-proposal -h
Submit a proposal along with an initial deposit.
Proposal title, description, type and deposit can be given directly or through a proposal JSON file.

Example:
  evochaincli tx gov submit-proposal --proposal="path/to/proposal.json" --from mykey
...
Which is equivalent to:
  ...
  evochaincli tx gov submit-proposal --title="Test Proposal" --description="My awesome proposal" --type="Text" \
	--deposit="10evo" --from mykey

Usage:
  evochaincli tx gov submit-proposal [flags]
  evochaincli tx gov submit-proposal [command]
...
Use "evochaincli tx gov submit-proposal [command] --help" for more information about a command.
```
##### Parameter description：
...
| --home              | account name and the directory where evochaincli is configured
if it is ~/.evochaincli can ignore the parameters |
...
```shell
$ evochaincli tx gov  submit-proposal param-change -h
...
Example:
  evochaincli tx gov submit-proposal param-change <path/to/proposal.json> --from=<key_or_address>
...
Usage:
  evochaincli tx gov submit-proposal param-change [proposal-file] [flags]

Flags:
  -a, --account-number uint     The account number of the signing account (offline mode only)
  -b, --broadcast-mode string   Transaction broadcasting mode (sync|async|block) (default "sync")
      --dry-run                 ignore the --gas flag and perform a simulation of a transaction, but don't broadcast it
      --fees string             Fees to pay along with transaction; eg: 1 evo
      --from string             Name or address of private key with which to sign
      --gas string              gas limit to set per-transaction; set to "auto" to calculate required gas automatically (default 200000) (default "200000")
      --gas-adjustment float    adjustment factor to be multiplied against the estimate returned by the tx simulation; if the gas limit is set manually this flag is ignored  (default 1)
      --gas-prices string       Gas prices to determine the transaction fee (e.g. 1evo)
...
| --deposit           | initial deposit specified when the proposal is initiated |
| --param             | specific parameters and values to be modified (gov/MinDeposit=1000evo
modify the MinDeposit parameters in the module to be governed to 1000evo. |
...
```shell
$ evochaincli tx gov deposit -h

Submit a deposit for an active proposal. You can
find the proposal-id by running "<appcli> query gov proposals".

Example:
  evochaincli tx gov deposit 1 10evo --from mykey

Usage:
  evochaincli tx gov deposit [proposal-id] [deposit] [flags]
...
Example:
  evochaincli tx gov vote 1 yes --from mykey

Usage:
  evochaincli tx gov vote [proposal-id] [option] [flags]
...
```shell
$ evochaincli tx gov submit-proposal community-pool-spend -h

Example:
	evochaincli tx gov submit-proposal community-pool-spend <path/to/proposal.json> --from=<key_or_address>

Where proposal.json contains:
{
  "title": "Community Pool Spend",
  "description": "Pay me some evo!",
...
Usage: 
    evochaincli tx gov submit-proposal community-pool-spend [proposal-file] [flags]
...
```shell
$ evochaincli query gov proposal -h

Query details for a proposal. You can find the
proposal-id by running "<appcli> query gov proposals".

Example:
  evochaincli query gov proposal 1

Usage:
  evochaincli query gov proposal [proposal-id] [flags]
...
```shell
$ evochaincli query gov proposals -h

Query for a all proposals. You can filter the returns with the following flags.

Example:
  evochaincli query gov proposals --depositor evo14tytg8a9fascnr4ruhszlflunyew5rw9qclx4n
  evochaincli query gov proposals --voter evo14tytg8a9fascnr4ruhszlflunyew5rw9qclx4n
  evochaincli query gov proposals --status (DepositPeriod|VotingPeriod|Passed|Rejected)

Usage:
  evochaincli query gov proposals [flags]
...
```shell
evochaincli query params params -h

Query the all the parameters for the governance process.

Example:
  evochaincli query params params

Usage:
  evochaincli query params params [flags]
...
```sh
evochaincli tx slashing unjail -h
unjail a jailed validator:

Example:
    evochaincli tx slashing unjail --from mykey

Usage:
    evochaincli tx slashing unjail [flags]
Flags:
    --from string             Name or address of private key with which to sign
```
...
```sh
evochaincli query slashing signing-info -h

Use a validators' consensus public key to find the signing-info for that validator:

Example:
    evochaincli query slashing signing-info evovalconspub1zcjduepqfhvwcmt7p06fvdgexxhmz0l8c7sgswl7ulv7aulk364x4g5xsw7sr0k2g5

Usage:
    evochaincli query slashing signing-info [validator-conspub] [flags]
...
```sh
evochaincli query slashing params -h

Query genesis parameters for the slashing module:

Example:
    evochaincli query slashing params

Usage:
    evochaincli query slashing params [flags]
...
Set the evochaind log level  

```shell
$ evochaincli debug set-loglevel -h

Example：
  $ evochaincli debug set-loglevel "main:info,state:info"

  $ evochaincli debug set-loglevel "upgrade:error"

Usage:
  evochaincli debug set-loglevel [flags]
...
Global Flags:
      --chain-id string   Chain ID of tendermint node
  -e, --encoding string   Binary encoding (hex|b64|btc) (default "hex")
      --home string       directory for config and data (default "/Users/yourname/.evochaincli")
  -o, --output string     Output format (text|json) (default "text")
      --passwd string     Pass word of sender (default "12345678")
      --trace             print out full stack trace on errors
```
...
```shell
$ evochaincli debug dump -h
Dump the data of kv-stores by a module name

Usage:
  evochaincli debug dump [module] [flags]
