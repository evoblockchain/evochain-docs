# CosmWasmJS
This article focuses on how to use CosmoWasmJS for connectiong to EVO nodes and interacting with smart contracts. CosmWasmJS is an SDK that facilitates contract development for DApp developers. Combined with CosmJS, you can easily and conveniently develop EVOWasm smart contracts.
...
### Mainnet
There is no need to build nodes on the main network, and the development of EVOWasm contracts can directly access the RPC node services provided by EVO. If you need to build your own testnet node, please refer to [mainnet node set up](../getting-start/join-evo-mainnet.html).

Below is the URL of the mainnet RPC node
> const rpcEndpoint = "https://evochaintmrpc.evoblock.com"
### Testnet

There is no need to build nodes on testnet, and the development of EVOWasm contracts can directly access the RPC node services provided by EVO. If you need to build your own testnet node, please refer to [testnet node set up](../getting-start/join-evo-testnet.html).

Below is the URL of the testnet RPC node
> const rpcEndpoint = "https://evochaintesttmrpc.evoblock.com"
### Local testnet
...
// If you choice mainnet ,please use "https://evochaintmrpc.evoblock.com"
// If you choice mainnet ,please use "https://evochaintesttmrpc.evoblock.com"
...
const accs = await wallet.getAccounts();
    ...
    const balance = await client.getBalance(accs[0].address,'evo')
    console.log("------------------------------------------------------------------------------------")
...
The above only shows how to connect EVO nodes through CosmWasmJS, the actions for querying account balances and all wasm contract code operations. If you want to conduct other smart contract actions, you can refer to the interfaces in the table below for uploading contract codes, deploying contracts and interacting with contracts.
...
| Order | Sdk interface name | Description | EVO support | Host Client| Remarks |
| :-----| :---- | :---- | :---| :---- | :---------- |
...
| 27 | delegateTokens | -| N |SigningCosmWasmClient  | EVO does not support wasm call staking |
| 28 | undelegateTokens | -| N |SigningCosmWasmClient  | EVO does not support wasm call staking |
| 29 | withdrawRewards | -| N |SigningCosmWasmClient  | EVO does not support wasm call staking |
...
## Example
We provide a Webpack [demo](https://github.com/evoblockchain/comswasmjs-client) example. You can follow this example to interact with EVO through your browser and Keplr wallet using CosmWasmJS and Webpack.
