...
Since EVO uses Tendermint Core as it's consensus Engine and it's built with the Cosmos
...
PubSubAPI](https://geth.ethereum.org/docs/rpc/pubsub), EVO needs to cast the Tendermint
...
```bash
evochaind start --rest.laddr "tcp://localhost:8545" --wsport 8546 --chain-id <chain_id>
```
