...
github: https://github.com/evoblockchain/evochain-go-sdk

---

## 1 utils function

All utils functions are defined in the package `utils` under path `evochain-go-sdk/utils`. They can be invoked by the way like:

```go
import "github.com/evoblockchain/evochain-go-sdk/utils"
...
|  Name   | Type  |Mark|
|  ----  | ----  |----|
| str  | string |transfer unit info in string, e.g. "addr1 1evo\naddr2 2evo"|
...
All auth functions are defined in the package `auth` under path `evochain-go-sdk/module/auth`. They can be invoked by the way like:

```go
import "github.com/evoblockchain/evochain-go-sdk"

config, _ := gosdk.NewClientConfig(rpcURL, chainID, gosdk.BroadcastBlock, "0.02evo", 200000, "")
...
All token functions are defined in the package `token` under path `evochain-go-sdk/module/token`. They can be invoked by the way like:
...
| coinsStr  | string |transfer amount, e.g. "1024.1024evo" |
...
| accNum  | uint64 |account number of sender's account on chain|
| seqNum  | uint64 |sequence number of sender's account on chain|
| coinsStr  | string | amount to mint, e.g. "1024.1024evo" |
...
| accNum  | uint64 |account number of sender's account on chain|
| seqNum  | uint64 |sequence number of sender's account on chain|
| coinsStr  | string | amount to burn, e.g. "1024.1024evo" |
...
All staking functions are defined in the package `staking` under path `evochain-go-sdk/module/staking`. They can be invoked by the way like:
...
#### 4.2.4 Deposit an amount of evo to delegator account
...
| accNum  | uint64 |account number of sender's account on chain|
| seqNum  | uint64 |sequence number of sender's account on chain|
| coinsStr  | string |amount to deposit, e.g. "1024.1024evo"|
...
#### 4.2.5 Withdraw an amount of evo and the corresponding shares from all validators
...
| accNum  | uint64 |account number of sender's account on chain|
| seqNum  | uint64 |sequence number of sender's account on chain|
| coinsStr  | string |amount to withdraw, e.g. "1024.1024evo"|
...
All distribution functions are defined in the package `distribution` under path `evochain-go-sdk/module/distribution`. They can be invoked by the way like:
...
All slashing functions are defined in the package `slashing` under path `evochain-go-sdk/module/slashing`. They can be invoked by the way like:
...
All dex functions are defined in the package `dex` under path `evochain-go-sdk/module/dex`. They can be invoked by the way like:
...
#### 7.2.4 Deposit evo to a specific product
...
| amountStr  | string |amount of evo to deposit to the product |
...
#### 7.2.5 Withdraw evo from a specific product
...
| seqNum  | uint64 |sequence number of sender's account on chain|
| product  | string |the name of target token pair|
| amountStr  | string |amount of evo to withdraw from the product |
...
All order functions are defined in the package `order` under path `evochain-go-sdk/module/order`. They can be invoked by the way like:
...
```go
	_, _ = cli.Order().NewOrders(info, defaultPassWd, "usdk_evo,eth_evo,btc_evo", "SELL,BUY,SELL", "2,3,4", "1024.1024,50.001,50.001", memo, accAccountNumber, accSequenceNumber)
```
...
All order functions are defined in the package `backend` under path `evochain-go-sdk/module/backend`. They can be invoked by the way like:
...
config, _ := gosdk.NewClientConfig(rpcURL, chainID, gosdk.BroadcastBlock, "0.02evo", 200000, "")
cli := gosdk.NewClient(config)
_, _ = cli.Backend().QueryCandles("usdk_evo", 60, 10)
```
...
All ammswap functions are defined in the package `ammswap` under path `evochain-go-sdk/module/ammswap`. They can be invoked by the way like:
...
All farm functions are defined in the package `farm` under path `evochain-go-sdk/module/farm`. They can be invoked by the way like:
...
All governance functions are defined in the package `governance` under path `evochain-go-sdk/module/governance`. They can be invoked by the way like:
...
```json
{
  ...
  "recipient": "evo1hcngft7gfkhn8z8fnlajzh7agyt0az0v6ztmme",
  "amount": [
    {
      "denom": "evo",
      "amount": "10.24"
...
All tendermint functions are defined in the package `tendermint` under path `evochain-go-sdk/module/tendermint`. They can be invoked by the way like:
