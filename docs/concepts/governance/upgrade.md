...
```sh
$ evochaincli tx gov submit-app-upgrade-proposal --title="app upgrade" --description="app upgrade, version 1" --deposit="50evo" --version=1 --software="http://newappdownloadingurl" --switchHeight="1000" --threshold="0.8" --type="AppUpgrade" --from jack -b block
...
```
EVO will generate a proposal-id to uniquely label the successfully submitted proposal. The proposal_id of the proposal in this example is 1. Query proposal information:
```sh
...
```
The current proposal status is DepositPeriod. The upgrade requires a deposit of 100evo during configuration. Only 50evo is deposited in the above proposal application, so that a deposit of over 50evo should be deposited again to enter the voting period. If the query results after initiating the proposal are:
```sh
...
```sh
$ evochaincli tx gov deposit 1 50evo --from jack -b block
...
```sh
$ evochaincli tx gov vote 1 yes --from jack -b block
...
Once the app upgrade proposal is approved, the administrator of each node can obtain version `1` and restart the background program of `EVO` via the download address specified by `--software="http://newappdownloadingurl"` in the proposal.
#### Query the current app version
```sh
$ evochaincli query upgrade version
{
  "version": "1"
}
```
...
```sh
$ evochaincli query upgrade failed-version
{
  "version": "0"
}
```
