...
This document describes the upgrade procedure of a `evochaind` full-node to a new version.

## Software Upgrade

First, stop your instance of `evochaind`. Next, upgrade the software:
...
See the [testnet repo](https://github.com/evoblockchain/testnets) for details on which version is needed for which public testnet, and the [EVO release page](https://github.com/evoblockchain/evochain/releases) for details on each release.
...
If you are joining a public testnet, fetch the genesis from the appropriate testnet in the [testnet repo](https://github.com/evoblockchain/testnets). Otherwise, fetch it from your trusted source.

Save the new genesis as `new_genesis.json`. Then replace the old `genesis.json` with `new_genesis.json`

```bash
cd $HOME/.evochaind/config
...
```bash
cd $HOME/.evochaind/config
evochaind export --for-zero-height --height=<export-height> > new_genesis.json
```
...
If you are running a **validator node** on the mainnet, always be careful when doing `evochaind unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
